...
# Function to generate test weather and bike data
...
# Call the test_weather_data_generation function
...
shinyServer(function(input, output){
  ...
  cities_max_bike <- city_weather_bike_df %>%
    select(CITY_ASCII, LNG, LAT, BIKE_PREDICTION, BIKE_PREDICTION_LEVEL,
    ...
    if (input$city_dropdown != 'All') {
      ...
      output$bike_line <- renderPlot({
        ggplot(
          ...
          geom_line(color = "lightblue", size = 1, linetype = "dashed") +
          geom_point() +
          geom_text() +
          theme(axis.text.x = element_blank())
      })
  ...
  output$city_bike_map <- renderLeaflet({
    ...
    leaflet(data = cities_max_bike) %>%
      ...
      addCircleMarkers(data = cities_max_bike,
                       ...
                       radius = ~ifelse(cities_max_bike$BIKE_PREDICTION_LEVEL == 'small', 6, 0),
